...
class DrugList(CMResource):
	...
	def get(self):
		...
		return redirect(self.api.url_for(NodesDetailed, h_id=self.default_h_id, n_type='drug'))
...
class Drug(CMResource):
	...
	def get(self,id):
		...
		return redirect(self.api.url_for(NodesByName, h_id=self.default_h_id, n_type='drug', name=id))
