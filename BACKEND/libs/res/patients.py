...
from .nodes import NodesDetailed, NodesByName, EdgesFromNodesByName
...
class PatientList(CMResource):
	...
	def get(self):
		...
		return redirect(self.api.url_for(NodesDetailed, h_id=self.default_h_id, n_type='patient'))
...
class Patient(CMResource):
	...
	def get(self, id:str):
		...
		return redirect(self.api.url_for(NodesByName, h_id=self.default_h_id, n_type='patient', name=id))
...
class PatientsMapGenes(CMResource):
	...
	def get(self,ids:str):
		...
		return redirect(self.api.url_for(EdgesFromNodesByName, h_id=self.default_h_id, n_type='patient', e_type='patient_gene_maps', name=ids))
...
class PatientsMapDrugs(CMResource):
	...
	def get(self,ids):
		...
		return redirect(self.api.url_for(EdgesFromNodesByName, h_id=self.default_h_id, n_type='patient', e_type='patient_drug_maps', name=ids))
...
ROUTES = CMRoutes(
	...
	routes=[
		...
		CMResPath(PatientTest, '/test/<splitpath(sub=list,subtype=int,sep=","):ruta>/b'),
